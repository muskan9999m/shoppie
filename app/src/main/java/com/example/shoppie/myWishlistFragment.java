...
public class myWishlistFragment extends Fragment {
   ...
   //     List<wishList_model> wishListModelList=new ArrayList<>();

        if(wishListModelLiist.size()==0)
        {
            DBqueries.Wishlist.clear();
            DBqueries.loadWishlist(getContext(),lodingDialog,true);
        }
        else
        {lodingDialog.dismiss();}
        wishlistadapter=new wishlist_adapter(wishListModelLiist,true);
